...
success:
	...
	if ((oldflags & (VM_WRITE | VM_SHARED | VM_LOCKED)) == VM_LOCKED &&
			(newflags & VM_WRITE)) {
		pr_info("populate_vma_page_range start = %lx end = %lx", start, end);
...
{
	// pr_info("do_mprotect_pkey start = %lx end = %lx pkey = %d", start, start+len, pkey);
	...
	// 	khugepaged_release_reservation(current->mm, addr);
