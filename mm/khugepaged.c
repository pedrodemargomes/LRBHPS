	...
	unsigned long address)
{
	...
	hash_for_each_possible(mm->thp_reservations->res_hash, res, node, haddr) {
		if (res->haddr == haddr)
			break;
	}
	return res;
...
void khugepaged_free_reservation(struct mm_struct *mm, struct thp_reservation *res)
{
	struct page *page;
	int unused;
	int i;

	// pr_info("khugepaged_free_reservation page_to_pfn(page) = %lx page_count(page) = %d PageCompound(page) = %d res->haddr = %lx vma->vm_flags = %lx Caller is %pS", page_to_pfn(res->page), page_count(res->page), PageCompound(res->page), res->haddr, res->vma->vm_flags,__builtin_return_address(0));
	...
	if (!PageCompound(page)) {
		for (i = 0; i < HPAGE_PMD_NR; i++) {
			// if (page_count(page+i) == 0)
			// 	pr_info("khugepaged_free_reservation page_count(page) == 0 page_to_pfn(page) = %lx", page_to_pfn(page+i));
			put_page(page + i);
			// page_ref_sub(page + i, 1);
		}
...
void khugepaged_reserve(struct mm_struct *mm, struct vm_area_struct *vma, unsigned long address)
{
	unsigned long haddr = address & HPAGE_PMD_MASK;
	...
	if (khugepaged_find_reservation(mm, address)) {
	 ...
	 */
	gfp = (GFP_HIGHUSER_MOVABLE | __GFP_NOMEMALLOC | __GFP_NOWARN | __GFP_RESERVE | __GFP_DIRECT_RECLAIM | __GFP_NORETRY) & ~__GFP_KSWAPD_RECLAIM & ~__GFP_COMP;
	page = folio_page(vma_alloc_folio(gfp, HPAGE_PMD_ORDER, vma, haddr, false), 0);
	// if(page)
	// 	pr_info("khugepaged_reserve page_to_pfn(page) = %lx haddr = %lx PageCompound(page) = %d PageTransHuge(page) = %d vma->vm_start = %lx vma->vm_end = %lx vma->vm_flags = %lx", page_to_pfn(page), haddr, PageCompound(page), PageTransHuge(page), vma->vm_start, vma->vm_end, vma->vm_flags);
	if (unlikely(!page)) {
		// pr_info("khugepaged_reserve failed allocation haddr = %lx ", haddr);
	...
	if (!res) {
		count_vm_event(THP_RES_ALLOC_FAILED);
		// pr_info("khugepaged_reserve out of mem");
				    ...
				    unsigned long address)
{
	...
	// pr_info("khugepaged_release_reservation address = %lx page_to_pfn(page) = %lx page_count(page) = %d PageCompound(page) = %d Call %pS", res->haddr, page_to_pfn(res->page), page_count(res->page), PageCompound(res->page),  __builtin_return_address(0));

	khugepaged_free_reservation(mm, res);
 ...
 */
void __khugepaged_release_reservations(struct mm_struct *mm,
				       unsigned long addr, unsigned long len)
{
	...
	hash_for_each_safe(mm->thp_reservations->res_hash, i, tmp, res, node) {
...
void thp_reservations_mremap(struct mm_struct *mm, struct vm_area_struct *vma,
		...
		bool need_rmap_locks)
{
	...
	if ((new_addr & ~HPAGE_PMD_MASK) != (old_addr & ~HPAGE_PMD_MASK)) {
		__khugepaged_release_reservations(mm, old_addr, len);
		return;
	}
	...
	if (old_addr & ~HPAGE_PMD_MASK) {
		res = khugepaged_find_reservation(mm, old_addr);
		if (res)
			khugepaged_free_reservation(mm, res);
	}
	...
	if (eaddr & ~HPAGE_PMD_MASK) {
		res = khugepaged_find_reservation(mm, eaddr);
		if (res)
			khugepaged_free_reservation(mm, res);
	}
	...
	hash_for_each_safe(mm->thp_reservations->res_hash, i, tmp, res, node) {
		...
		res->vma = new_vma;
		res->haddr += offset;
		hash_add(mm->thp_reservations->res_hash, &res->node, res->haddr);
	}
					 ...
					 void *cb_arg)
{
	...
	//if(!cb_arg)
	//	pr_info("shrink thp_lru_free_reservation");
	if (PageCompound(res->page))
	...
	if (cb_arg)
		*((unsigned long *)cb_arg) = page_to_pfn(page);
...
// 		if (PageCompound(page)) {
// 			pr_info("__collapse_huge_page_isolate PageCompound(page) page_to_pfn(page) = %d", page_to_pfn(page));
// 			struct page *p;
				      ...
				      spinlock_t *ptl)
{
	...
	for (_pte = pte, idx_end = 0; _pte < pte + HPAGE_PMD_NR;
				_pte++, page++, address += PAGE_SIZE, idx_end++) {
		pte_t pteval = *_pte;
		set_page_count(page, 0);
		// TestClearPageLRU(page);
		// INIT_LIST_HEAD(&page->lru);
		// pr_info("__collapse_huge_page_convert page_to_pfn(page) = %lx page_count(page) = %lx PageLRU(page) = %d pte_none(pteval) = %d is_zero_pfn(pte_pfn(pteval)) = %d", page_to_pfn(page), page_count(page), PageLRU(page), pte_none(pteval), is_zero_pfn(pte_pfn(pteval)));
					...
					pte_t *pte)
{
	...
	for (_pte = pte; _pte < pte+HPAGE_PMD_NR;
		...
		if (PageCompound(page)) {
			// pr_info("__collapse_huge_page_isolate PageCompound(page) page_to_pfn(head) = %lx PageCompound(head) = %d PageLRU(head) = %d", page_to_pfn(page), PageCompound(page), PageLRU(page));
			goto out;
		}
		...
		if (!trylock_page(page)) {
			// pr_info("__collapse_huge_page_isolate !trylock_page(page) page_to_pfn(head) = %lx PageCompound(head) = %d PageLRU(head) = %d", page_to_pfn(page), PageCompound(page), PageLRU(page));
			goto out;
		}
		...
		if (isolate_lru_page(page)) {
			unlock_page(page);
			// pr_info("isolate_lru_page(page) error page_to_pfn(page) = %lx PageLRU(page) = %d", page_to_pfn(page), PageLRU(page));
			goto out;
		}
...
out:
	// pr_info("release_pte_pages");
	release_pte_pages(pte, _pte, NULL);
	return result;
}
...
static int __collapse_huge_page_swapin(struct mm_struct *mm,
				       ...
				       int referenced, struct thp_reservation *res)
{
	int swapped_in = 0;
	vm_fault_t ret = 0;
...
// 			      struct collapse_control *cc, struct thp_reservation *res)
// {
...
// 			pr_info("collapse_huge_page failed address = %lx page_to_pfn(page) = %lx is_reserved = %d", address, page_to_pfn(res->page), is_reserved);
// 			goto out_up_write;
// 		}
...
// 	spin_lock(pte_ptl);
// 	result =  __collapse_huge_page_isolate(vma, address, pte, res, cc,
// 					       &compound_pagelist);
// 	spin_unlock(pte_ptl);
...
// 		spin_unlock(pmd_ptl);
// 		anon_vma_unlock_write(vma->anon_vma);
// 		pr_info("collapse_huge_page failed address = %lx page_to_pfn(page) = %lx is_reserved = %d", address, page_to_pfn(hpage), is_reserved);
// 		goto out_up_write;
// 	}
...
// 	// pr_info("collapse_huge_page page_to_pfn(hpage) = %lx address = %lx", page_to_pfn(hpage), address);

// 	if (is_reserved) {
// 		pr_info("__collapse_huge_page_convert page_to_pfn(page) = %lx", page_to_pfn(hpage));
// 		__collapse_huge_page_convert(pte, hpage, vma, address, pte_ptl);
// 		// for (i = 0; i < HPAGE_PMD_NR; i++)
// 		// 	pr_info("collapse_huge_page page_to_pfn(hpage) = %lx page_count(hpage) = %d PageTransHuge(hpage) = %d is_transparent_hugepage(hpage) = %d PageCompound(hpage) = %d", page_to_pfn(hpage+i), page_count(hpage+i), PageTransHuge(hpage+i), is_transparent_hugepage(hpage+i), PageCompound(hpage+i));
// 		khugepaged_free_reservation(vma, res);
// 	} else {
...
// 	// for (i = 0; i < HPAGE_PMD_NR; i++)
// 	// 	pr_info("> collapse_huge_page page_to_pfn(hpage) = %lx page_count(hpage) = %d page_ref_count(page) = %d PageTransHuge(hpage) = %d is_transparent_hugepage(hpage) = %d PageCompound(hpage) = %d", page_to_pfn(hpage+i), page_count(hpage+i), page_ref_count(hpage+i), PageTransHuge(hpage+i), is_transparent_hugepage(hpage+i), PageCompound(hpage+i));

// 	pr_info("collapse_huge_page page_to_pfn(hpage) = %lx address = %lx is_reserved = %d", page_to_pfn(hpage), address, is_reserved);
...
int promote_huge_page_address(struct mm_struct *mm, unsigned long haddr)
{
	...
	if (!vma || !vma_is_anonymous(vma) || haddr < vma->vm_start || haddr + HPAGE_PMD_SIZE > vma->vm_end)
		goto out;

	result = hugepage_vma_revalidate_old(mm, haddr, &vma);
	if (result)
		goto out;

	struct thp_reservation *res = khugepaged_find_reservation(mm, haddr);
	if(!res || !(bitmap_weight(res->mask, 512) > hugepage_promotion_threshold)) {
		goto out;
	}

	head = res->page;
	if (PageCompound(head)) {
		// pr_info("PageCompound page_to_pfn(head) = %d", page_to_pfn(head));
		goto out;
	}

	// pr_info("promote_huge_page_address page_to_pfn(head) = %lx", page_to_pfn(head));
	...
	// pr_info("before promote_huge_page_address haddr = %lx PageCompound(head) = %d page_to_pfn(head) = %lx page_count(page) = %d", haddr, PageCompound(head), page_to_pfn(head), page_count(head));
	// for (int i = 0; i < 512; i++)
	// 	pr_info("before promote_huge_page_address page_to_pfn(res->page + i) = %lx page_count(res->page + %d) = %d page_mapcount(res->page) = %d PageCompound(page) = %d compound_mapcount_ptr(page) = %d PageLRU(page) = %d", page_to_pfn(res->page + i), i, page_count(res->page + i), page_mapcount(res->page + i), PageCompound(res->page + i), atomic_read(compound_mapcount_ptr(res->page + i)), PageLRU(res->page + i));

	// if (vma)
	// 	pr_info("promote_huge_page_address haddr = %lx page_to_pfn(head) = %lx vma->vm_start = %lx vma->vm_end = %lx vma->vm_flags = %lx", haddr, page_to_pfn(head), vma->vm_start, vma->vm_end, vma->vm_flags);
	...
	spin_lock(pte_ptl);
	// pr_info("__collapse_huge_page_isolate haddr = %lx PageCompound(head) = %d page_to_pfn(head) = %lx", haddr, PageCompound(head), page_to_pfn(head));
	isolated = __collapse_huge_page_isolate(vma, haddr, pte);
	spin_unlock(pte_ptl);
	...
	__collapse_huge_page_convert(pte, new_page, vma, haddr, pte_ptl);
	...
	lru_cache_add_inactive_or_unevictable(head, vma);
	pgtable_trans_huge_deposit(mm, pmd, pgtable);
	...
	// pr_info("after promote_huge_page_address page_to_pfn(res->page + i) = %lx page_count(res->page) = %d page_mapcount(res->page) = %d PageCompound(page) = %d PageTransHuge(page) = %d compound_mapcount_ptr(page) = %d PageLRU(page) = %d PageHead(page) = %d", page_to_pfn(res->page), page_count(res->page), page_mapcount(res->page), PageCompound(res->page), PageTransHuge(res->page), atomic_read(compound_mapcount_ptr(res->page)), PageLRU(res->page), PageHead(res->page));
	// for (int i = 0; i < 512; i++)
				   ...
				   struct collapse_control *cc)
{
	...
	// pr_info("hpage_collapse_scan_pmd address = %lx", address);

	result = find_pmd_or_thp_or_none(mm, address, &pmd);
	// if (result != SCAN_SUCCEED) {
		// pr_info("hpage_collapse_scan_pmd find_pmd_or_thp_or_none address = %lx", address);
		goto out;
	// }
	...
	for (_address = address, _pte = pte; _pte < pte + HPAGE_PMD_NR;
		...
		// res = khugepaged_find_reservation(vma, address);
		// if (res) {
		// 	// pr_info("hpage_collapse_scan_pmd page_to_pfn(page) = %lx is_refcount_suitable = %d", page_to_pfn(page), is_refcount_suitable(page));
...
out_unmap:
	...
	if (result == SCAN_SUCCEED) {
		// pr_info("Call collapse_huge_page address = %lx", address);
			    ...
			    bool install_pmd)
{
	...
	// pr_info("before collapse_pte_mapped_thp haddr = %lx PageCompound(head) = %d page_to_pfn(head) = %lx page_count(page) = %d", haddr, PageCompound(res->page), page_to_pfn(res->page), page_count(res->page));
	// for (int i = 0; i < 512; i++)
	// 	pr_info("before collapse_pte_mapped_thp page_to_pfn(res->page + i) = %lx page_count(res->page + %d) = %d page_mapcount(res->page) = %d PageCompound(page) = %d compound_mapcount_ptr(page) = %d PageLRU(page) = %d", page_to_pfn(res->page + i), i, page_count(res->page + i), page_mapcount(res->page + i), PageCompound(res->page + i), atomic_read(compound_mapcount_ptr(res->page + i)), PageLRU(res->page + i));
...
drop_hpage:
	...
	// for (int i = 0; i < 512; i++)
	// 	pr_info("after promote_huge_page_address page_to_pfn(hpage + i) = %lx page_count(hpage + %d) = %d page_mapcount(hpage + i) = %d PageCompound(hpage=i) = %d compound_mapcount_ptr(hpage+i) = %d PageLRU(hpage+i) = %d", page_to_pfn(hpage+i), i, page_count(hpage + i), page_mapcount(hpage+i), PageCompound(hpage+i), atomic_read(compound_mapcount_ptr(hpage+i)), PageLRU(hpage+i));
