							...
							bool check_target)
{
	...
	// pr_info("__reset_isolation_pfn = %ld", pfn);
	...
	do {
		if (check_source && PageLRU(page)) {
			// pr_info("clear_pageblock_skip check_source");
			clear_pageblock_skip(page);
			return true;
		}

		if (check_target && PageBuddy(page)) {
			// pr_info("clear_pageblock_skip check_target");
			clear_pageblock_skip(page);
			return true;
		}
							...
							unsigned long pfn)
{
	...
	if (!skip && !cc->no_set_skip_hint) {
		// pr_info("test_and_set_skip page = %lx", page_to_pfn(page));
			...
			struct page *page, unsigned long pfn)
{
	...
	// pr_info("update_pageblock_skip page = %lx", page_to_pfn(page));
	set_pageblock_skip(page);
...
isolate_migratepages_block(struct compact_control *cc, unsigned long low_pfn,
			unsigned long end_pfn, isolate_mode_t mode, bool freed)
{
	...
	for (; low_pfn < end_pfn; low_pfn++) {
		...
		if (!valid_page && pageblock_aligned(low_pfn)) {
			if (!freed && !isolation_suitable(cc, page)) {
				low_pfn = end_pfn;
				//pr_info("isolation_suitable(cc, page) page = %lx", page_to_pfn(page));
				page = NULL;
				goto isolate_abort;
			}
			valid_page = page;
		...
		if (!PageLRU(page)) {
			...
			// pr_info("!PageLRU(page) page_to_pfn(page) = %lx page_count(page) = %d", page_to_pfn(page), page_count(page));
			goto isolate_fail;
		}
		...
		if (unlikely(!get_page_unless_zero(page))) {
			// pr_info("get_page_unless_zero page_to_pfn(page) = %lx page_count(page) = %d", page_to_pfn(page), page_count(page));
			goto isolate_fail;
		}
...
isolate_success:
		// pr_info("isolate_success page_to_pfn(page) = %lx", page_to_pfn(page));
...
isolate_abort:
	// pr_info("isolate_abort nr_isolated = %d", nr_isolated);
	if (locked)
		unlock_page_lruvec_irqrestore(locked, flags);
	if (page) {
		SetPageLRU(page);
	...
	if (low_pfn == end_pfn && (!nr_isolated || cc->rescan)) {
		if (valid_page && !skip_updated) {
			// pr_info("isolate_migratepages_block page = %lx", page_to_pfn(valid_page));
...
fast_isolate_around(struct compact_control *cc, unsigned long pfn)
{
	...
	if (cc->nr_freepages < cc->nr_migratepages) {
		// pr_info("fast_isolate_around page = %lx", page_to_pfn(page));
		set_pageblock_skip(page);
	}
 ...
 */
static unsigned long fast_find_migrateblock(struct compact_control *cc, bool break_reservs, bool *freed)
{
	...
	*freed = false;
	if (break_reservs) {
		...
		// else {
		// 	pr_info("freed failed pfn_reserv = %lx cc->free_pfn = %lx zone = %s cc->order = %d", pfn_reserv, cc->free_pfn, cc->zone->name, cc->order);
		// }
	}
	...
	for (order = cc->order - 1;
		...
		list_for_each_entry(freepage, freelist, lru) {
			...
			if (free_pfn < high_pfn) {
				...
				cc->fast_search_fail = 0;
				// pr_info("fast_find_migrateblock page = %lx", page_to_pfn(freepage));
				found_block = true;
	...
	// else {
	// 	pr_info("fast_find_migrateblock found block pfn = %lx", pfn);
	// }
	return pfn;
}
...
static isolate_migrate_t isolate_migratepages(struct compact_control *cc)
{
	 ...
	 */
	low_pfn = fast_find_migrateblock(cc, true, &freed);
	...
	for (; block_end_pfn <= cc->free_pfn;
		...
		page = pageblock_pfn_to_page(block_start_pfn,
						block_end_pfn, cc->zone);
		if (!page) {
			// pr_info("!page block_start_pfn = %lx block_end_pfn = %lx", block_start_pfn, block_end_pfn);
			continue;
		}
		...
		if (pageblock_aligned(low_pfn) &&
			...
			// pr_info("fast_find_block = %d", fast_find_block);
			continue;
		}
		...
		if (!suitable_migration_source(cc, page)) {
			// pr_info("suitable_migration_source");
		...
		if (isolate_migratepages_block(cc, low_pfn, block_end_pfn,
						isolate_mode, freed)) {
			// if (freed) {
			// 	pr_info("isolate_migratepages_block freed error");
			// }
			return ISOLATE_ABORT;
		}

		// if (freed && !cc->nr_migratepages) {
		//  	pr_info("no migrated pages freed error");
		// }
					...
					unsigned long wmark_target)
{
	...
	if (zone_watermark_ok(zone, order, watermark, highest_zoneidx,
								alloc_flags)) {
		//if (order == 9)
			//pr_info("zone_watermark_ok COMPACT_SUCCESS");
		return COMPACT_SUCCESS;
	}
	...
	if (!__zone_watermark_ok(zone, 0, watermark, highest_zoneidx,
		...
		// pr_info("!__zone_watermark_ok watermark = %ld order = %d highest_zoneidx = %d wmark_target = %ld COMPACT_SKIPPED", watermark, order, highest_zoneidx, wmark_target);
...
compact_zone(struct compact_control *cc, struct capture_control *capc)
{
	...
	while ((ret = compact_finished(cc)) == COMPACT_CONTINUE) {
		...
		// pr_info("nr_succeeded = %ud", nr_succeeded);

		// if (cc->proactive_compaction) {
		// 	cc->nr_migrated_pages += nr_succeeded;
		// 	//pr_info("nr_migrated_pages = %d", nr_migrated_pages);
		// 	// Migrate max of 1Gb/s of memory per compaction
		// 	if (cc->nr_migrated_pages > 12800) {
		// 		pr_info("Limit of 12800 pages of memory exceded nr_migrated_pages = %d > 12800",
		// 			cc->nr_migrated_pages);
		// 		ret = COMPACT_SUCCESS;
		// 		break;
		// 	}
		// }
		...
		enum compact_priority prio, struct page **capture)
{
	...
	for_each_zone_zonelist_nodemask(zone, z, ac->zonelist,
		...
		if (prio > MIN_COMPACT_PRIORITY
			...
			// pr_info("try_to_compact_pages continue zone = %s", zone->name);
			continue;
		}
			...
			void *buffer, size_t *length, loff_t *ppos)
{
	...
	if (((char *)buffer)[0] == 's') {
		for (int zoneid = 0; zoneid < MAX_NR_ZONES; zoneid++) {
			...
			unsigned long tmp_pfn = pageblock_start_pfn(start_pfn);
			// pr_info("start_pfn = %lu end_pfn = %lu tmp_pfn = %lu", start_pfn, pageblock_start_pfn(end_pfn-1), tmp_pfn);
			for (; tmp_pfn < pageblock_start_pfn(end_pfn-1); tmp_pfn += 1) {
				// struct page *p = pageblock_pfn_to_page(tmp_pfn, tmp_pfn+pageblock_nr_pages, cc->zone);
				struct page *p = pfn_to_online_page(tmp_pfn);
				if (!p) {
					// pr_info("p = NULL");
					continue;
				}
				// pr_info("p = %lx tmp_pfn = %lu page_count(p) = %d", p, tmp_pfn, page_count(p));
				if (get_pageblock_skip(p))
					c++;
				clear_pageblock_skip(p);
			}
			// pr_info("zone = %s num pageblocks skip = %lu\n", zone->name, c);
		}
		return 0;
	}
