			...
			unsigned int order, bool check_free, fpi_t fpi_flags)
{
	...
	if (unlikely(PageHWPoison(page)) && !order) {
		pr_info("unlikely(PageHWPoison(page)) && !order page_to_pfn(page) = %lx", page_to_pfn(page));
		/*
...
restart:
	...
	// 		if (t > 3000) {
	// 			// pr_info("__alloc_pages_slowpath t = %u", t);
	// 			should_free++;
	// 		}
	// 		// if (thp_lru_free_reservation(pos, l, &nlru->lock, 1000) == LRU_REMOVED_RETRY) {
	...
	// 	if (should_free > 0)
	// 		pr_info("__alloc_pages_slowpath list_size = %d num_freed = %d should_free = %d", list_size, num_freed, should_free);
...
retry:
	...
	// pr_info("__alloc_pages_slowpath __alloc_pages_direct_reclaim order = %d nlru->nr_items = %ld", order, nlru->nr_items);
...
{
	struct page *page;
	if (gfp &__GFP_RESERVE)
		page = __alloc_pages(gfp, order,
			preferred_nid, nodemask);
	else
		page = __alloc_pages(gfp | __GFP_COMP, order,
			preferred_nid, nodemask);

	if (page && order > 1 && gfp && !(gfp &__GFP_RESERVE))
