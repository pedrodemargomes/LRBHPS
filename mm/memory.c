	...
	do {
		...
		if (pte_present(ptent)) {
			...
			if (unlikely(!should_zap_page(details, page))) {
				// pr_info("zap_pte_range should_zap_page page_to_pfn(page) = %lx", page_to_pfn(page));
				continue;
			}
			...
			zap_install_uffd_wp_if_needed(vma, addr, pte, details,
						      ptent);
			if (unlikely(!page)) {
				// pr_info("zap_pte_range addr = %lx", addr);
				continue;
			}
			...
			if (unlikely(page_mapcount(page) < 0)) {
				// pr_info("zap_pte_range page_mapcount(page) < 0");
				...
				struct zap_details *details)
{
	...
	do {
		...
		if (is_swap_pmd(*pmd) || pmd_trans_huge(*pmd) || pmd_devmap(*pmd)) {
			// pr_info("is_swap_pmd(*pmd) || pmd_trans_huge(*pmd) || pmd_devmap(*pmd) addr = %lx", addr);
			if (next - addr != HPAGE_PMD_SIZE) {
				// if (mm_thp_reservations(vma->vm_mm)) {
				// 	pr_info("addr = %lx", addr);
				...
				// 	if(res) {
				// 		pr_info("zap_pmd_range addr = %lx", addr);
				// 		for (i = 0; i < 512; i++)
				// 			pr_info("zap_pmd_range page_to_pfn(res->page + i) = %lx page_count(res->page + %d) = %d page_mapcount(res->page) = %d PageCompound(page) = %d compound_mapcount_ptr(page) = %d PageLRU(page) = %d", page_to_pfn(res->page + i), i, page_count(res->page + i), page_mapcount(res->page + i), PageCompound(res->page + i), atomic_read(compound_mapcount_ptr(res->page + i)), PageLRU(res->page + i));
				// 	}
				// }
		...
		if (pmd_none_or_trans_huge_or_clear_bad(pmd)) {
			// pr_info("pmd_none_or_trans_huge_or_clear_bad addr = %lx", addr);
			goto next;
		}

		// if (vma_thp_reservations(vma)) {
		// 	mutex_lock(&vma->thp_reservations->res_hash_lock);
		// 	struct thp_reservation *res = khugepaged_find_reservation(vma, addr);
		// 	if (res) {
		// 		for (i = 0; i < 512; i++)
		// 			pr_info("page_to_pfn(res->page + i) = %lx page_count(res->page + %d) = %d page_mapcount(res->page) = %d PageCompound(page) = %d compound_mapcount_ptr(page) = %d PageLRU(page) = %d", page_to_pfn(res->page + i), i, page_count(res->page + i), page_mapcount(res->page + i), PageCompound(res->page + i), atomic_read(compound_mapcount_ptr(res->page + i)), PageLRU(res->page + i));
		// 	}
		...
		if (mm_thp_reservations(vma->vm_mm)) {
			...
			if (res) {
				...
				// pr_info("zap_pmd_range page_to_pfn(res->page) = %lx PageCompound(res->page) = %d vma->vm_start = %lx vma->vm_end = %lx addr = %lx next = %lx", page_to_pfn(res->page), PageCompound(res->page), vma->vm_start, vma->vm_end, addr, next);

				// for (i = 0; i < 512; i++)
				// 	pr_info("after zap_pte_range page_to_pfn(res->page + i) = %lx page_count(res->page + %d) = %d page_mapcount(res->page) = %d PageCompound(page) = %d compound_mapcount_ptr(page) = %d PageLRU(page) = %d", page_to_pfn(res->page + i), i, page_count(res->page + i), page_mapcount(res->page + i), PageCompound(res->page + i), atomic_read(compound_mapcount_ptr(res->page + i)), PageLRU(res->page + i));
				...
				if (bitmap_weight(res->mask, 512) == 0) {
					...
					// for (i = 0; i < 512; i++) {
					// 	if (PageLRU(p+i))
					// 		pr_info("zap_pte_range bitmap_weight(res->mask, 512) == 0 page_to_pfn(res->page + i) = %lx page_count(res->page + %d) = %d page_mapcount(res->page) = %d PageCompound(page) = %d compound_mapcount_ptr(page) = %d PageLRU(page) = %d", page_to_pfn(p + i), i, page_count(p + i), page_mapcount(p + i), PageCompound(p + i), atomic_read(compound_mapcount_ptr(p + i)), PageLRU(p + i));
					// }
...
static vm_fault_t do_anonymous_page(struct vm_fault *vmf)
{
	...
	if (!page) {
		...
		// pr_info("clear_user_highpage page_to_pfn(page) = %lx vmf->address = %lx page_count(page) = %d", page_to_pfn(page), vmf->address, page_count(page));
	...
	vmf->pte = pte_offset_map_lock(vma->vm_mm, vmf->pmd, vmf->address,
			&vmf->ptl);
	if (!pte_none(*vmf->pte)) {
		// pr_info("!pte_none(*vmf->pte) page_to_pfn(page) = %lx page_count(page) = %d page_mapcount(page) = %d PageLRU(page) = %d PageCompound(page) = %d vmf->address = %lx", page_to_pfn(page), page_count(page), page_mapcount(page), PageLRU(page), PageCompound(page), vmf->address);
		update_mmu_tlb(vma, vmf->address, vmf->pte);
		goto release;
	}
	...
	ret = check_stable_address_space(vma->vm_mm);
	if (ret) {
		// pr_info("check_stable_address_space page_to_pfn(page) = %lx page_count(page) = %d page_mapcount(page) = %d PageLRU(page) = %d PageCompound(page) = %d vmf->address = %lx", page_to_pfn(page), page_count(page), page_mapcount(page), PageLRU(page), PageCompound(page), vmf->address);
		goto release;
	}
	/* Deliver the page fault to userland, check inside PT lock */
	if (userfaultfd_missing(vma)) {
		// pr_info("userfaultfd_missing(vma) page_to_pfn(page) = %lx", page_to_pfn(page));
		pte_unmap_unlock(vmf->pte, vmf->ptl);
		put_page(page);
		return handle_userfault(vmf, VM_UFFD_MISSING);
	}
